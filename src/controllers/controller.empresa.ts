...
async function InserirFavorito(req, res) {
    try {
        ...
        const empresas = await serviceEmpresas.InserirFavorito(id_usuario, id_empresa);
...
async function ExcluirFavorito(req, res) {
    try {
        const id_usuario = req.id_usuario;
        const id_empresa = req.params.id_empresa;
        const empresas = await serviceEmpresas.ExcluirFavorito(id_usuario, id_empresa);
...
async function ListarProdutoId(req, res) {
    try {
        ...
        const produto = await serviceEmpresas.ListarProdutoId(id_empresa, id_produto);
...
export default { Destaques, Listar, InserirFavorito, ListarProdutoId, ExcluirFavorito, Cardapio };